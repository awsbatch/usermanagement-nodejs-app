pipeline {
    ...
    stages {
        ...
        stage('test') {
            steps {
                sh 'sleep 2'
                sh 'echo "Hit http://192.168.1.59:5000 to see the app."'
            }
        }
        stage ('cleanup') {
            steps {
    ...
    post {
        ...
        {
         always {
            echo "Build ${env.BUILD_NUMBER} completed at ${new Date().format("yyyy-MM-dd HH:mm:ss")}"
         }
        }
    }
}
